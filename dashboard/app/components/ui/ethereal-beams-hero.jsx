...
function extendMaterial(BaseMaterial, cfg) {
  ...
  Object.entries(cfg.vertex || {}).forEach(([inc, code]) => {
    vertexShader = vertexShader.replace(inc, `${inc}\n${code}`);
  });
  Object.entries(cfg.fragment || {}).forEach(([inc, code]) => {
    fragmentShader = fragmentShader.replace(inc, `${inc}\n${code}`);
  });
  ...
  return new THREE.ShaderMaterial({
    ...
    fragmentShader,
    lights: true,
    fog: Boolean(cfg.material && cfg.material.fog),
  });
}
...
const DirLight = ({ position, color }) => {
  ...
  useEffect(() => {
    ...
    const cam = dir.current.shadow ? dir.current.shadow.camera : null;
    if (cam) {
      cam.top = 24;
      cam.bottom = -24;
      cam.left = -24;
    ...
    if (dir.current.shadow) dir.current.shadow.bias = -0.004;
  }, []);
...
export function Beams({
  ...
  // WebGL cannot exist during server render — bail out entirely.
